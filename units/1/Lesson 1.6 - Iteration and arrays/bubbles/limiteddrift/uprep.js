...
function moveBubble(bubble) {
    if (bubble.frames > 0) {
        bubble.dx = bubble.dx + randomBetween(-100, 100) / 100;
        bubble.dy = bubble.dy + randomBetween(-100, 100) / 100;
        bubble.frames = bubble.frames - 1;
    }
    bubble.x = bubble.x + bubble.dx;
    bubble.y = bubble.y + bubble.dy;
    //bubble.radius = bubble.radius * 0.99;
}
