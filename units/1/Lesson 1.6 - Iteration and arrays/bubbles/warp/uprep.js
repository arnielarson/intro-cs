...
function addBubble(canvas) {
    ...
    bubble.x = randomBetween(0, canvas.width);
    bubble.y = randomBetween(0, canvas.height);
...
function isBubbleRunningAway(bubble, canvas) {
    ...
    var distanceX = Math.abs(centerX - bubble.x);
    var distanceY = Math.abs(centerY - bubble.y);
...
function moveBubble(bubble) {
    ...
    bubble.dx = (canvas.width / 2 - bubble.x) / 10;
    bubble.dy = (canvas.width / 2 - bubble.y) / 10;
    
    
    bubble.radius = bubble.radius * 0.9;
